...
// The little "verified on X" mark under the reviewer. Purely a label — it
// says where the review was collected, and only shows when one is set.
function SourceMark({ source, dark }) {
  if (!source) return null;
  const google = /google/i.test(source);
  return (
    <span className={`mt-1.5 flex items-center gap-1 text-[11px] ${dark ? 'text-slate-500' : 'text-gray-400'}`}>
      {google && (
...
function Card({ item, dark, index, onRemove, removing }) {
  const { name, rating = 5, text, media = [], date, mine, source } = item;
  return (
    <motion.figure
    ...
    >
      ...
      <figcaption className="mt-3.5 flex items-center gap-2.5">
        ...
        <span className="min-w-0 flex-1">
          ...
          <SourceMark source={source} dark={dark} />
        </span>
        {mine && onRemove && (
          <button
            type="button"
