...
export default function StorefrontLayout() {
  ...
  const Brand = (
    <Link to="/" className="flex min-w-0 items-center gap-2" aria-label={storeName}>
      {settings.logoUrl ? (
        <img src={settings.logoUrl} alt={storeName} className="h-9 w-auto shrink-0 object-contain sm:h-11" />
      ) : (
        <>
  ...
  // silently breaks `position: sticky` for the header and any sticky section
  // inside a page. `clip` stops sideways scroll the same way but keeps sticky.
  return (
